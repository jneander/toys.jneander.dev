...
function renderRow (datum, rowIndex, grid) {
  ...
  return (
    <tr key={rowIndex}>
      <td className={styles.GridCell} ref={refFor(0)} tabIndex={ tabIndexFor(0) }>{ datum.date }</td>
      <td className={styles.GridCell} ref={refFor(1)} tabIndex={ tabIndexFor(1) }>{ datum.type }</td>
      <td className={styles.GridCell}>
        <a ref={refFor(2)} tabIndex={ tabIndexFor(2) } href="#">{ datum.description }</a>
      </td>
      <td className={styles.GridCell} ref={refFor(3)} tabIndex={ tabIndexFor(3) }>{ datum.amount }</td>
      <td className={styles.GridCell} ref={refFor(4)} tabIndex={ tabIndexFor(4) }>{ datum.balance }</td>
    </tr>
  );
}
