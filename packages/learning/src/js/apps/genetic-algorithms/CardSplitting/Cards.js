  ...
  render () {
    ...
    return (
      <div className={styles.Container}>
        <Heading level="h3" margin="0 0 small 0">{ this.props.label }</Heading>

        <div className={styles.CardSet__Top}>
          {
            sumGenes.map((card, index) => (
              <span key={index} className={styles.Card}>{ card }</span>
            ))
          }

          <span className={styles.SetMetrics}>
            Sum: {geneSum(this.props.chromosome)}
          </span>
        </div>
        ...
        <div className={styles.CardSet__Bottom}>
          ...
          <span className={styles.SetMetrics}>
            Product: {geneProduct(this.props.chromosome)}
          </span>
        </div>
      </div>
    );
